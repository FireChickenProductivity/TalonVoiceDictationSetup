...
@context.capture("user.prose_simple_number", rule="{user.fire_chicken_dictation_number_prefix} <user.number_string>")
def prose_simple_number(m) -> str:
    return m.number_string


@context.capture("user.prose_number_with_dot", rule="{user.fire_chicken_dictation_number_prefix} <user.number_string> (dot | point) <digit_string>")
def prose_number_with_dot(m) -> str:
    return m.number_string + "." + m.digit_string


@context.capture("user.prose_number_with_colon", rule="{user.fire_chicken_dictation_number_prefix} <user.number_string> colon <user.number_string>")
